import { world, system, BlockPermutation, ItemStack, EnchantmentTypes, EntityTypes, PlayerPermissionLevel } from "@minecraft/server";
...
function saveDrops() {
  try {
    const data = activeDrops.map(d => ({ x: d.x, y: d.y, z: d.z, block: d.block, p: d.platformId }));
...
function loadDrops() {
  try {
    ...
    for (const d of data) {
      ...
      activeDrops.push({ x: d.x, y: d.y, z: d.z, block: d.block, platformId: d.p });
    }
...
function setBlockFast(dim, x, y, z, typeId) {
  try {
...
const BLACKOUT_DROP_RATE = 999999;
function computeDropRate(category, wave) {
...
function stepDrops() {
  ...
  if (dropsDirty && state.dropTick % 10 === 0) {
    saveDrops();
    dropsDirty = false;
  }
}
...
function handleCommand(player, cmd, arg, fullMsg) {
  ...
  if (cmd === "wave") {
    ...
    if (!arg || arg === "all") {
      ...
      for (const idStr of Object.keys(platformState)) {
        const p = getPlatformById(parseInt(idStr));
        if (p) { clearAbovePlatformById(p); buildPlatformById(p); delete colHeightCache[p.id]; }
      }
      ...
      buildPlatformById(platform);
      delete colHeightCache[platform.id];   // stale heights would mis-bias the next drops
    ...
    if (!isNaN(num) && num >= 1 && num <= 9) {
      ...
      for (const [k, d] of Object.entries(platformState)) {
    ...
    if (p) {
      ...
      buildPlatformById(p);
      delete colHeightCache[p.id];
    }
    delete platformState[idStr];
    savePlatformState();
    ...
    [
      "§e--- FallingFalling v1.9.1 Commands ---",
...
function gameTick() {
  ...
  if (state.tick % 100 === 75 && structureState.length < MAX_STRUCTURES) {
